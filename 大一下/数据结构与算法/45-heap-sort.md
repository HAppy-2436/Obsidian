...
二叉堆基础
 介绍过二叉堆结构，
二叉堆实现优先级队列
 利用二叉堆结构实现了一个 SimpleMinPQ 优先级队列，插入队列的元素会按照从小到大的顺序取出。
...
O(NlogN)，其中 N 是输入数组的长度。

这个思路可以得到正确的排序结果，但空间复杂度是
𝑂
(
𝑁
)
O(N)，因为我们创建的这个优先级队列是一个额外的数据结构，它的底层使用了一个数组来存储元素。
