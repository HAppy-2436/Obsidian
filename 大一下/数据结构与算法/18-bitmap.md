...
- 理解位图（BitMap）用 bit 位存储整数存在性的核心思想
- 掌握位运算（`<<`、`>>`、`|`、`&`、`~`）和位图地址映射
- 用 `std::vector<bool>` 或 `std::vector<uint8_t>` 实现一个完整的位图
- 能在 10 亿用户去重、排序加速等场景中合理选用位图

## 🎯 一句话总结

位图是一种用「bit 位」标记整数存在性的紧凑数据结构——N 个整数的存在性只需 N 个 bit（约 N/8 字节），相比布尔数组节省 7/8 的内存。
