...
- **堆排序（Heap Sort）**：见 [[45-堆排序|堆排序]]，基于 `push` + `pop` 或原地 heapify 实现，复杂度 O(N log N)、原地排序。
- **优先级队列（Priority Queue）**：见 [[22-二叉堆实现|优先级队列实现]]，任务调度、Dijkstra 算法、Huffman 编码都需要它。
- **Top K 问题**：维护一个大小为 K 的小顶堆，可在 O(N log K) 内找到第 K 大/小的元素。
- **流式数据的中位数**：用一个大顶堆 + 一个小顶堆，可以在 O(log N) 内维护中位数。

## 下一章

→ [[22-二叉堆实现|堆排序/优先级队列的代码实现]]

## 相关章节

- [[02-数组原理|数组原理]] — 二叉堆的底层存储结构
- [[23-二叉树基本概念|二叉树基础]] — 二叉堆的逻辑结构
- [[45-堆排序|堆排序]] — 二叉堆最重要的应用之一
- [[27-二叉搜索树原理|二叉搜索树原理]] — 另一种「动态排序」数据结构，对比学习
