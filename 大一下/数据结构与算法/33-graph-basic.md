...
- 理解图结构是**多叉树结构**的延伸
- 掌握**邻接表**和**邻接矩阵**两种存储方式
- 理解**加权/无权、有向/无向**这四种组合的存储差异
- 能够手写一个**通用的 Graph 类**，对外屏蔽邻接表/邻接矩阵的差异
- 掌握从**边列表（Edge List）**加载图的方法
- 了解 [[34-graph-traverse-basic|DFS/BFS 遍历]] 和后续算法（最短路、MST、欧拉图）如何使用该 Graph 接口

## 一句话总结

图结构 = **多叉树 + 环的可能性**；常用的两种存储方式——**邻接表（Adjacency List，空间 O(V+E)）** 和 **邻接矩阵（Adjacency Matrix，空间 O(V²)）**——我们用一套**统一的 Graph API** 封装它们，后续所有图算法（DFS/BFS/Dijkstra/Prim/Hierholzer）都基于这套 API 编写，做到**算法代码与存储方式解耦**。
...
- [[25-n-ary-tree-traverse-basic|N 叉树的递归/层序遍历]]（逻辑上的延伸关系）

## 1. 图 = 多叉树的延伸

在[[23-binary-tree-basic|树结构]]中，只允许**父节点指向子节点**，不存在子节点指向父节点，子节点之间也不会互相链接；而**图**没有那么多限制，节点之间可以**相互指向**，形成复杂的网络结构。

| 结构 | 关系 | 是否有环 | 典型表示 |
|------|------|----------|----------|
| **多叉树** | 严格的父子关系 | **无环** | 父节点 + children 列表 |
| **图** | 任意节点间都可连接 | **可能有环** | 邻接表 / 邻接矩阵 |

正因为图可能**有环**，我们[[34-graph-traverse-basic|遍历图]]时必须用 `visited` 数组防止死循环——这是图与树最本质的区别。

## 2. 两种存储方式

### 2.1 邻接表（Adjacency List）

**核心思想**：用**链表/向量**为**每个节点维护一个邻居列表**。`adj[v]` 存储**所有从 v 出发的边**。
...
**空间复杂度**：O(V + E)，适合**稀疏图**（绝大多数实际场景）。

### 2.2 邻接矩阵（Adjacency Matrix）

**核心思想**：维护一个 V × V 的二维数组 `matrix`；`matrix[u][v] = 1`（无权）或 `w`（加权 w）表示存在边 `u -> v`。
...
**空间复杂度**：O(V²)，适合**稠密图**，且能 O(1) 判定两节点是否相邻。
...
|------|--------|----------|
| 空间 | **O(V+E)** | O(V²) |
| 加边 | O(1)（追加到列表） | O(1)（赋值即可） |
| 删边 | O(deg(v)) | O(1) |
| 查询两节点是否相邻 | O(deg(v)) | **O(1)** |
| 遍历 v 的邻居 | O(deg(v)) | O(V) |
| 适合的图 | **稀疏图**（绝大多数） | 稠密图、点数 V ≤ 1000 |
| 典型算法 | DFS / BFS / 拓扑排序 / Kruskal | Floyd / Prim（稠密版） |

## 3. 通用的 Edge 结构

无论是邻接表还是邻接矩阵，都用 `Edge` 描述一条**出边**：
