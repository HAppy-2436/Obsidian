...
category: 图结构基础及算法概览
order: 34
prerequisites: [33-图结构实现, 24-二叉树遍历]
group: 图 / 遍历
paywall: true
...
为了和[[25-N叉树遍历|多叉树遍历]]直观对比，先用一个"伪 Vertex" 类：
...
实际工程里我们用 [[33-图结构实现|Graph 接口]] 写，更通用：
...
2. **遍历顺序**：BFS 在无权图中得到的访问顺序就是**从起点出发的层序**——这是 BFS 找**最短路**的基础（见 [[35-DFS和BFS应用|DFS/BFS 应用]]）。
...
> [!warning] 性能警告
> 二维 visited 数组占用 **O(V²) 空间**，对大图很浪费。[[38-欧拉图|Hierholzer 算法]] 会给出更聪明的优化（不依赖二维数组）。
...
- **BFS**：无权图最短路、扩散、层序访问
- 详细应用见 [[35-DFS和BFS应用|DFS 和 BFS 的常用场景]]

## 下一章

→ [[35-DFS和BFS应用|DFS 和 BFS 的常用场景]]

## 相关章节

- [[33-图结构实现|图结构通用实现]] — 遍历的目标对象
- [[24-二叉树遍历|二叉树遍历]] — 图遍历的逻辑原型
- [[25-N叉树遍历|N 叉树遍历]] — 另一种"无环"的多叉遍历
- [[35-DFS和BFS应用|DFS/BFS 应用]] — 遍历的具体用例
- [[37-最短路径算法|最短路径]] — BFS 的扩展
- [[38-欧拉图|欧拉图]] — 遍历所有边的应用
