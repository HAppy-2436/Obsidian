...
1. **哈希映射 + 多次位图**：用多个 hash 函数把 int64 映射到 [0, m)，即 [[19-布隆过滤器实现|布隆过滤器]]。
...
这其实就是 [[47-计数排序|计数排序]] 的"位图版"——区别在于计数排序用 `int[] count` 记录每个值出现多少次，位图用 `bool[]` 只关心是否出现。
...
3. **不支持范围统计**。想知道"值在 [L, R] 内的元素有几个"，位图需要遍历 O(R-L+1) 个 bit；用线段树或 [[47-计数排序|计数排序]] 用的 `int[]` 更合适。
...
- 位图排序（值域小时比快速排序更快）
- IP 黑名单 / 恶意 URL 过滤（与 [[19-布隆过滤器实现|布隆过滤器]] 互补）
- 操作系统页表、磁盘块管理
- 数据库 bitmap 索引（加速 `IN` 查询）

## ▶️ 下一章

[[19-布隆过滤器实现|布隆过滤器原理和实现]]

## 🔗 相关章节

- [[17-哈希集合实现|哈希集合的原理及代码实现]]
- [[19-布隆过滤器实现|布隆过滤器原理和实现]]
- [[47-计数排序|全新的排序原理：计数排序]]
