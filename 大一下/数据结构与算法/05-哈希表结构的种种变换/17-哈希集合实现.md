...
| --- | --- | --- |
| 判断元素是否存在（如 [[19-布隆过滤器实现|布隆过滤器]] 的退化版） | ✅ | ❌ |
| 维护「元素 → 计数」或「元素 → 另一个值」 | ❌ | ✅ |
...
注意 `Slot::DELETED` 这个"墓碑标记"——这是线性探查法支持删除的关键，详见 [[14-线性探查法难点|线性探查法的两个难点]]。
...
- **作为 [[19-布隆过滤器实现|布隆过滤器]] 的退化版**：当数据规模不大、内存够时，哈希集合比布隆过滤器更精确（零误判）。
...
- [[11-哈希表核心原理|哈希表核心原理]]
- [[16-拉链法实现哈希表|拉链法实现哈希表]]
- [[15-线性探查法实现|线性探查法的两种代码实现]]
- [[18-位图原理和实现|位图原理和实现]]
