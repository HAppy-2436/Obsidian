<!-- TAG: 32-graph-terminology | URL: https://labuladong.online/zh/algo/data-structure-basic/graph-terminology/ -->
...
V
2
，我们说这幅图是 稠密图（Dense Graph）；如果只有很少的边，即 
𝐸
E 远小于 
𝑉
2
...
′
 是 
𝐺
G 的一个子图。简单来说，子图是从原图中删除一些节点和边后得到的图。

假设上面这幅图为 
𝐺
G，我们举例说明子图的概念。子图有两种特殊类型：

生成子图 (Spanning Subgraph)：包含原图中所有节点，但只包含部分边的子图。

下图是图 
𝐺
G 的一个生成子图，它包含了所有节点，但移除了节点 3 和节点 4 之间的边。

导出子图 (Induced Subgraph)：选择原图的一部分节点，以及这些节点之间在原图中的所有边所构成的子图。

下图是图 
𝐺
G 的一个导出子图，它包含节点 1,2,3,4 及它们之间在原图中的所有边。
