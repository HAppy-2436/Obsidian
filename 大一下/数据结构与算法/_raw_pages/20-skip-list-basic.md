<!-- TAG: 20-skip-list-basic | URL: https://labuladong.online/zh/algo/data-structure-basic/skip-list-basic/ -->
...
在 
链表基础
 中我们说到，在单链表中增删查改指定索引的元素所需的时间复杂度是 
𝑂
(
𝑁
)
...
O(1) 的时间直接拿到目标节点，避免了遍历查找的时间消耗，这个思路在后面的 
哈希链表（LinkedHashMap）
 中会详细介绍。
...
⁡
𝑁
logN 层索引，在每层索引中移动的次数不会超过 2 次（因为上层索引区间在下一层被分为两半），所以跳表的查询时间复杂度是 
𝑂
(
log
⁡
...
1、上面的例子只展示了查询操作，但跳表肯定得支持插入和删除操作，这就涉及到索引层中节点的动态调整，你需要保证每一层的索引区间尽可能二分，这样才能保证索引层的高度为 
log
⁡
𝑁
logN，否则时间复杂度就会退化。
