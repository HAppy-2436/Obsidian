...
order: 44
prerequisites: [24-二叉树遍历, 43-归并排序]
group: 排序 / 分治
paywall: true
...
4. **不适用于**：
   - 需要稳定排序时（用 [[大一下/数据结构与算法/10-十大排序算法原理及可视化/43-归并排序|归并]]）
   - 数据几乎有序且不能随机化（用堆排）
...
**期望时间复杂度 O(N)**，比 [[45-堆排序|堆排]] 的 O(N log K) 更优。

## 下一章

→ [[45-堆排序|堆排序]]：利用二叉堆实现原地 O(N log N) 排序，避免快排的最坏 O(N²)。

## 相关章节

- [[大一下/数据结构与算法/10-十大排序算法原理及可视化/43-归并排序|归并排序]] — 后序位置的另一种分治排序
- [[42-选择排序|选择排序]] — 快排的优化起点
- [[45-堆排序|堆排序]] — 解决快排最坏情况的备选
- [[21-二叉堆原理|二叉堆基础]] — 堆排的前置
- [[22-二叉堆实现|优先级队列]]
...
- [[大一下/数据结构与算法/10-十大排序算法原理及可视化/43-归并排序|利用后序位置：归并排序]]
