...
- 掌握 **DFS** 的常见应用：**连通块、岛屿、排列、拓扑序**
- 掌握 **BFS** 的常见应用：**无权图最短路、扩散、层序**
- 理解 **DFS 适合穷举所有路径**、**BFS 适合求最短路** 的本质原因
- 掌握 [[34-graph-traverse-basic|visited / onPath]] 在不同场景下的使用方式
- 能够独立解决 5 道 LeetCode 经典题目

## 一句话总结

**DFS = 递归 + 栈**，擅长"一条路走到黑"，适合**穷举所有可能（路径/连通块/排列）**；**BFS = 队列**，擅长"一圈圈扩散"，适合**求最短路/扩散问题**——本质是 DFS 的"层序访问"对应到 BFS 的队列逐层出队。

## 前置知识

阅读本文前，你需要先学习：

- [[24-binary-tree-traverse-basic|二叉树的递归/层序遍历]]（DFS/BFS 的最简形态）
- [[34-graph-traverse-basic|图结构的 DFS/BFS 遍历]]（图上的通用遍历模板）

## 1. 为什么 DFS 适合"穷举所有路径"？

[[24-binary-tree-traverse-basic|二叉树递归遍历]]本质就是 DFS——递归隐含**调用栈**，天然支持"回溯"——所以 DFS 的标志性能力是**回溯**：进入时做选择，撤销时撤回选择。

**典型场景**：全排列、组合、子集、路径枚举、N 皇后。
...
[[24-binary-tree-traverse-basic|二叉树层序遍历]]本质就是 BFS——队列天然支持"先入先出"，所以**先入队的节点就先被处理**。
...
> **进阶**：本题也可直接用[[30-union-find-basic|并查集]]做。
...
- [[24-binary-tree-traverse-basic|二叉树遍历]] — DFS/BFS 的最简形态
- [[34-graph-traverse-basic|图遍历]] — DFS/BFS 的图版本
- [[30-union-find-basic|并查集]] — 求连通分量的另一种思路
- [[37-graph-shortest-path|最短路径]] — BFS 的加权扩展（Dijkstra）
- [[36-graph-minimum-spanning-tree|最小生成树]] — 图论核心应用
