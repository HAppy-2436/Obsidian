...
普通 [[23-二叉树基本概念|二叉树]] 要找一个节点，必须 **遍历整棵树**，复杂度 O(N)。而 BST 利用「左小右大」的特性，**每访问一个节点就能砍掉一半的搜索空间**，类似二分查找：
...
解决退化问题的方案就是 **自平衡 BST**——[[31-红黑树原理|红黑树]]、AVL 树等。本节我们先讲普通 BST，下一节再讲自平衡版本。
...
普通 [[05-链表原理|链表]] 的 `ListNode` 只能存「值 + next 指针」；BST 的 `TreeNode` 需要存「值 + 左指针 + 右指针」；TreeMap 的节点需要存「键 + 值 + 左指针 + 右指针」：
...
> [!info] 📌 概念演示
> 下面给出一个 **普通 BST 版本** 的 TreeMap 简化实现（**未做自平衡**，最坏情况会退化为 O(N)）。完整、可直接使用的版本在 [[28-二叉搜索树实现|下一章]] 会基于红黑树给出。
...
> [!warning] 重要提示
> 这个 `SimpleTreeMap` 只是为了演示 BST 的基本 API，**不保证 O(log N)**。当数据有序插入时会退化成链表。要在工程中使用，请用 [[28-二叉搜索树实现|基于红黑树的 TreeMap]]，即 C++ STL 的 `std::map` / `std::set`。
...
- **求解「中位数 / 第 k 小」**：用两个堆（[[22-二叉堆实现|优先级队列]]）或一个增强型 BST。
- **LeetCode 经典题**：
...
→ [[28-二叉搜索树实现|TreeMap/TreeSet 的代码实现（基于红黑树）]]

## 相关章节

- [[23-二叉树基本概念|二叉树基础]] — BST 的逻辑结构
- [[31-红黑树原理|红黑树]] — 自平衡 BST，标准库 TreeMap 的底层
- [[11-哈希表核心原理|哈希表]] — 与 TreeMap 对比：哈希表无序但 O(1)，TreeMap 有序但 O(log N)
- [[29-字典树原理|Trie 树]] — TreeMap 键是字符串时的特化版本
- [[22-二叉堆实现|优先级队列]] — 另一种「按某种序维护元素」的数据结构

## 关联章节

- [[23-二叉树基本概念|二叉树基本概念及特殊二叉树]]
