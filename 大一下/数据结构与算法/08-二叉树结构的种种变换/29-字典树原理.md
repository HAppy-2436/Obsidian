...
也可以用 [[11-哈希表核心原理|哈希表]] 代替数组（节省空间，但常数更大）：
...
如果是 [[29-字典树原理|TrieMap]]（存键值对而非仅键），再加一个 `V value` 字段。

## 3. Trie 的核心优势

### 3.1 节约公共前缀的存储空间

举例，往 [[11-哈希表核心原理|HashMap]] 里插入 `"apple"`、`"app"`、`"appl"` 三个键：
...
TrieMap 天然支持「以前缀为单位的查询」，这在 [[11-哈希表核心原理|HashMap]] / [[27-二叉搜索树原理|TreeMap]] 上做要付出极高代价：
...
→ [[30-并查集原理|Union Find 并查集原理]]

## 相关章节

- [[25-N叉树遍历|N 叉树遍历]] — Trie 的逻辑结构
- [[27-二叉搜索树原理|BST / TreeMap]] — Trie 是 TreeMap 在「字符串 + 前缀」场景下的特化
- [[11-哈希表核心原理|HashMap]] — Trie 的「无前缀」替代品
- [[19-布隆过滤器实现|布隆过滤器]] — Trie 的概率版本
