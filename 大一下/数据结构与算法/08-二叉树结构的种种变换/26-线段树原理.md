...
category: 二叉树结构的种种变换
order: 26
prerequisites: [23-二叉树基本概念, 02-数组原理]
group: 高级树
paywall: true
...
- **区间求和 / 区间最值**：[[26-线段树原理|线段树]] 最经典的应用，比如「求数组 `nums[i..j]` 的元素和」或「滑动窗口最大值」。
...
> 如果你只需要 **「单点更新 + 区间求和」** 或 **「区间更新 + 单点查询」**，[[46-希尔排序|树状数组]]（BIT，Binary Indexed Tree）可以用 **一半的代码量** 实现同样 O(log N) 的复杂度。但如果你需要 **「区间更新 + 区间查询」** 或 **「区间最值 / 区间 GCD」** 等更复杂的聚合，线段树是更通用的选择。
...
- [[23-二叉树基本概念|二叉树基础]] — 线段树的逻辑结构
- [[02-数组原理|数组原理]] — 线段树用数组紧凑存储
- [[21-二叉堆原理|二叉堆]] — 另一种「用数组实现的树」，对比学习
- [[22-二叉堆实现|堆的代码实现]] — 树形结构的代码实现技巧
- [[45-堆排序|堆排序]] — 区间排序的另一种实现
