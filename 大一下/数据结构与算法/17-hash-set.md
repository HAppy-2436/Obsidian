...
- 理解哈希集合（Hash Set）与哈希表（Hash Map）的本质关系
- 掌握基于拉链法哈希表实现 HashSet 的完整 C++ 代码
- 对比 Java `HashSet` 源码与 C++ `std::unordered_set` 的内部原理
- 能够在算法题中选用合适的集合实现（集合 vs 映射）

## 🎯 一句话总结

哈希集合的本质就是「只关心键、不关心值」的哈希表——把哈希表的 `KVNode` 简化成 `KNode`，用 O(1) 时间完成增删查改并保证元素唯一。
...
在数据结构层面，「集合（Set）」和「映射（Map）」是一对孪生兄弟。映射存的是键值对（key-value），集合只存键（key）——集合中每个元素本身就是键，而值要么是固定占位（比如 Java `HashSet` 底层用 `HashMap`，把值设成同一个 `PRESENT` 对象），要么干脆不存。

从这个角度看，**实现 HashSet 的最简单方法就是：在 HashMap 之上做一层薄薄的封装**。所有的 `add/remove/contains` 操作都委托给底层 HashMap 的 `put/remove/getKey`。

labuladong 在原文中也直言：「哈希集合没什么好讲的，它就是把前文讲的哈希表简单封装了一下：哈希表的键，其实就是哈希集合。」尽管如此，作为基础章节中独立的一篇，我们还是有必要把原理、代码实现和工业级语言中的对应物完整过一遍。

### 基于拉链法哈希表实现 HashSet

我们直接复用 [[16-hashtable-chaining|拉链法哈希表]] 的结构，把节点类型从 `KVNode<K, V>` 简化为 `KNode<K>`：
